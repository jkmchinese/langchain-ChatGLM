...
def convert(issue, headers, project_id):
...
def crawlCore(
    ...
    page = 1
    while True:
        issue_url = f"http://git.rdapp.com/api/v4/projects/{project_id}/issues?state=all&iid=&labels=&milestone=&order_by=updated_at&sort=desc&page={page}&per_page=100&updated_after={updatedAfterTime}'"
        headers = {"PRIVATE-TOKEN": token}
        print(f"issue_url:{issue_url}")
        response = requests.get(issue_url, headers=headers).json()
        issues = list(
            map(lambda x: convert(x, headers=headers, project_id=project_id), response))
